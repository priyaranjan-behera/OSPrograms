...
main (int argc, char *argv[])
{
  ...
  char host[64];
  char portnum[64];
  int s, p[10], fp, rc, len, port, nplayers, nhops,nport;
  ...
  for(int i=0; i<nplayers; i++) {
    ...
    //while ( 1 ) { no need of whilte loop, we read just once
  ...
  {
    player.port = nport;
    player.right = nport+1;
    player.left = port+nplayers+1;
    plaer.id = i+1;
  }
  else
  {
