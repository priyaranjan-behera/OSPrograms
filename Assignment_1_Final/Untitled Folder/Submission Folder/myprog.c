...
typedef struct MyThreadHeavy {
    ucontext_t context;
    int id;
    struct MyThreadHeavy * next;
    int parent;
...
static MyThreadHeavy* ready_queue;
static MyThreadHeavy* blocked_queue;
static MyThreadHeavy* semaphore_blocked_queue;
static MySemaphore* semaphore_queue;

static ucontext_t controller;

void assert(void *a)
{
	if(a == NULL)
	{
		printf("Memory Exception: Exit");
		exit(0);
	}
}
...
//
		...
		while(trial != NULL)
		{	
			printf(" (%d - Waiting for %d) ", trial->id, trial->blockedFor[0]);
			trial = trial->next;
			cnt++;
		}
...
MyThreadHeavy* insertNode(MyThreadHeavy* head)
{
	...
	if(head == NULL)
	{
		head = malloc(sizeof(MyThreadHeavy));
		assert(&head);
		return head;
	}
	else
	{
		...
		trav->next = malloc(sizeof(MyThreadHeavy));
		assert(&trav->next);
		return trav->next;
	}
...
void insertIntoBlockedQueue(MyThreadHeavy* newNode)
{
...
void insertIntoSemaphoreBlockedQueue(MyThreadHeavy* newNode)
{
...
void insertIntoReadyQueue(MyThreadHeavy* newNode)
{
...
int MyThreadJoin(MyThread thread_light)
{
	MyThread thread = (MyThread)thread_light;
	...
	MyThreadHeavy* trav = ready_queue->next;
	MyThreadHeavy* temp = ready_queue;
...
{
	//printQueues();
	MyThreadHeavy* trav = ready_queue;
...
MyThreadHeavy* moveNextNode(MyThreadHeavy *head)
{
	head = head->next;
}
...
MyThread MyThreadCreate (void(*start_funct)(void *), void *args) //Finalizes Create Thread Function
{
	...
	MyThreadHeavy* child = insertNode(ready_queue);
	getcontext(&(child->context));
	child->context.uc_stack.ss_sp=malloc(MEM);
	assert(&child->context.uc_stack.ss_sp);
	...
	MyThread thr;
	thr.id = threadCount-1;
	thr.parent = ready_queue->id;
	return thr;
}
...
{
	MyThreadHeavy* trav = semaphore_blocked_queue;
...
void CheckForUnblocking()
{
	...
	//printf("CP1");
	//printf("Blocked Queue: %d", blocked_queue);
	//printQueues();

	//printf("CP2");
 	//printQueues();
	MyThreadHeavy* trav = blocked_queue;
	MyThreadHeavy* temp;
	if(trav == NULL)
	{
		return;
	...
	MyThreadHeavy* prev = blocked_queue;
	trav = blocked_queue;
	while(trav != NULL)
	{
		flag = 0;
