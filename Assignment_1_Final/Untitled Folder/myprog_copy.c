...
typedef struct MyThreadEx {
    ucontext_t context;
    int id;
    struct MyThreadEx * next;
    int parent;
...
MyThreadEx* ready_queue;
MyThreadEx* blocked_queue;
MyThreadEx* semaphore_blocked_queue;
...
MyThreadEx* insertNode(MyThreadEx* head)
{
	...
	else
	{
		...
		trav->next = malloc(sizeof(MyThreadEx));
		return trav->next;
	}

}

void insertIntoBlockedQueue(MyThreadEx* newNode)
{
...
void insertIntoSemaphoreBlockedQueue(MyThreadEx* newNode)
{
...
void insertIntoReadyQueue(MyThreadEx* newNode)
{
...
int MyThreadJoin(MyThread thread)
{
	...
	MyThreadEx* trav = ready_queue->next;
	MyThreadEx* temp = ready_queue;
...
MyThreadEx* moveNextNode(MyThreadEx *head)
{
	head = head->next;
}
...
MyThread MyThreadCreate (void(*start_funct)(void *), void *args) //Finalizes Create Thread Function
{
	...
	MyThreadEx* child = insertNode(ready_queue);
	...
	printf("\n The size of the return is: %d",sizeof(ret));
...
{
	MyThreadEx* trav = semaphore_blocked_queue;
...
start = 1;
	...
	MyThreadEx* node;
    	////printf("\nCP1");
	ready_queue = malloc(sizeof(MyThreadEx));
